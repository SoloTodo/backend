...
export default function AIAssociateTable({
    ...
    const handleAIAssociateSubmit = () => {
        ...
        jwtFetch(null, `https://api.solotodo.com/entities/${entity.id}/ai_associate/?llm_model=${defaultLlmModel}`, {
            method: "POST",
        }).then((data) => {
            setAIAssociate(data);
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={7}>
                                ...
                                <CustomTable
                                    ...
                                    rows={aiAssociate["ai_association_result"]["similar_product_entries"].map((entry: {
                                        [key: string]: any;
                                    }) => [
                                ...
                                <CustomTable
                                    ...
                                    rows={[
                                        [
                                            <NextLink
                                                href={`${PATH_PRODUCT.root}/${aiAssociate["ai_association_result"]["associated_product"]["id"]}`}
                                                passHref>
                                                <Link>{aiAssociate["ai_association_result"]["associated_product"]["name"]}</Link>
                                            </NextLink>,
                                            aiAssociate["ai_association_result"]["product_created"],
                                            <Link target="_blank"
                                                  href={`https://api.solotodo.com/metamodel/instances/${aiAssociate["ai_association_result"]["associated_product"]['instance_model_id']}`}>Link</Link>
                                        ]
                                    ]}
                                />
                            </Grid>
