...
import {Store, StoreSectionPositionsUpdateLog} from "../../../frontend-utils/types/store";
...
type StoreSectionPositionsUpdateLogPageProps = {
  update_logs_registry: StoreUpdateLogEntry[];
  update_log: StoreSectionPositionsUpdateLog;
}

// ----------------------------------------------------------------------

StoreSectionPositionsUpdateLogPage.getLayout = function getLayout(page: ReactElement) {
  return <Layout>{page}</Layout>;
};

// ----------------------------------------------------------------------

export default function StoreSectionPositionsUpdateLogPage(props: StoreSectionPositionsUpdateLogPageProps) {
  ...
  return (
    <Page title={`${store.name} > Registros de actualización de posicionamiento > ${props.update_log.id}`}>
      <Container maxWidth={false}>
          ...
          links={[
            ...
            { name: "Registros de actualización de posicionamiento", href: `${PATH_STORE.root}/${store.id}/section_positions_update_logs` },
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  const update_logs_registry = await jwtFetch(
    context,
    `${apiSettings.apiResourceEndpoints.store_section_positions_update_logs}${context.params?.id}/registry/`
  );
  const update_log = await jwtFetch(
    context,
    `${apiSettings.apiResourceEndpoints.store_section_positions_update_logs}${context.params?.id}/`
  );
  return {
    props: {
